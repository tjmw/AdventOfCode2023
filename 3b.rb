...
g.each_with_index do |row, row_idx|
    row.each_with_index do |tuple, column_idx|
        if (tuple.value == "*")
            g[row_idx-1]&.[](column_idx-1)&.gear_index = multiplier_counter
            g[row_idx-1]&.[](column_idx)&.gear_index = multiplier_counter
            g[row_idx-1]&.[](column_idx+1)&.gear_index = multiplier_counter
            g[row_idx]&.[](column_idx-1)&.gear_index = multiplier_counter
            g[row_idx]&.[](column_idx+1)&.gear_index = multiplier_counter
            g[row_idx+1]&.[](column_idx-1)&.gear_index = multiplier_counter
            g[row_idx+1]&.[](column_idx)&.gear_index = multiplier_counter
            g[row_idx+1]&.[](column_idx+1)&.gear_index = multiplier_counter
...
g.each do |line|
    ...
    line.each do |tuple|
        if /^\d$/.match(tuple.value)
            ...
            if !number_acc.empty? && number_acc.any? {|t| t.gear_index }
                matching_gear_indexes = number_acc.select(&:gear_index).map(&:gear_index).uniq
                matching_gear_indexes.each do |idx|
                    gear_map[idx] ||= []
                    gear_map[idx].push(number_acc.map(&:value).join("").to_i)
                end
            end
...
puts gear_map.values.inject(0) {|memo, arr|
    if arr.length == 2
        memo + arr[0] * arr[1]
    else
        memo
