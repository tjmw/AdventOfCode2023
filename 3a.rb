...
Tuple = Struct.new(:value, :next_to_symbol)

grid = DATA.each_line.each_with_object([]) { |line, memo|
    memo.push(line.chop.split(""))
}

symbol_regex = /^[^\d\.]$/

grid_with_indicators = grid.each_with_index.each_with_object([]) { |(row, row_idx), memo|
    row_with_indicators = row.each_with_index.each_with_object([]) do |(value, column_idx), memo|
        cell_next_to_symbol =
            (grid[row_idx-1]&.[](column_idx-1) && symbol_regex.match(grid[row_idx-1][column_idx-1])) ||
            (grid[row_idx-1]&.[](column_idx) && symbol_regex.match(grid[row_idx-1][column_idx])) ||
            (grid[row_idx-1]&.[](column_idx+1) && symbol_regex.match(grid[row_idx-1][column_idx+1])) ||
            (grid[row_idx]&.[](column_idx-1) && symbol_regex.match(grid[row_idx][column_idx-1])) ||
            (grid[row_idx]&.[](column_idx+1) && symbol_regex.match(grid[row_idx][column_idx+1])) ||
            (grid[row_idx+1]&.[](column_idx-1) && symbol_regex.match(grid[row_idx+1][column_idx-1])) ||
            (grid[row_idx+1]&.[](column_idx) && symbol_regex.match(grid[row_idx+1][column_idx])) ||
            (grid[row_idx+1]&.[](column_idx+1) && symbol_regex.match(grid[row_idx+1][column_idx+1]))

        memo.push(Tuple.new(value, !!cell_next_to_symbol))
    end

    memo.push(row_with_indicators)
}

numbers_next_to_symbols = grid_with_indicators.each_with_object([]) { |line, memo|
    number_acc = []

    line.each do |tuple|
        if /^\d$/.match(tuple.value)
            number_acc.push(tuple)
        else
            if !number_acc.empty? && number_acc.any? {|t| t.next_to_symbol }
                memo.push(number_acc.map(&:value).join("").to_i)
            end
    ...
    if number_acc.any? {|t| t.next_to_symbol }
        memo.push(number_acc.map(&:value).join("").to_i)
    end
}

puts numbers_next_to_symbols.sum
