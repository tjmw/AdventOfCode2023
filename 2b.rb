...
Maxes = Struct.new(:red, :green, :blue)

puts DATA.each_line.inject(0) { |sum, line|
    id_section, games = line.split(":")

    id = /\d+/.match(id_section).to_s

    maxes = games.split(";").inject(Maxes.new(0, 0, 0)) {|memo, game_string|
        red_count, = / (\d+) red/.match(game_string)&.values_at(1)
        blue_count, = / (\d+) blue/.match(game_string)&.values_at(1)
        green_count, = / (\d+) green/.match(game_string)&.values_at(1)

        red_count = red_count&.to_i || 0
        green_count = green_count&.to_i || 0
        blue_count = blue_count&.to_i || 0

        Maxes.new(
            [memo.red, red_count].max,
            [memo.green, green_count].max,
            [memo.blue, blue_count].max,
        )
    }
