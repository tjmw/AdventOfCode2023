...
num_regex = /(#{Regexp.union(map.keys.concat(map.values.map(&:to_s)))})/
reversed_num_regex = /(#{Regexp.union(map.keys.map(&:reverse).concat(map.values.map(&:to_s)))})/
num_string_regex = /#{Regexp.union(map.keys)}/

puts DATA.each_line.inject(0) { |sum, line|
    first = num_regex.match(line).to_s
    last = reversed_num_regex.match(line.reverse).to_s.reverse!

    first.gsub!(num_string_regex) { |s| map[s] }
    last.gsub!(num_string_regex) { |s| map[s] }
